...
class AuthService {
  private currentUser: ArtisanProfile | null = MOCK_USER;
  private isUserAuthenticated: boolean = true; // Set to true by default for developer ease, editable via auth actions
  ...
  async signup(params: SignUpParams): Promise<{ success: boolean; user?: ArtisanProfile; message?: string }> {
    await new Promise((resolve) => setTimeout(resolve, 800));
  ...
  async verifyOTP(params: VerifyOTPParams): Promise<{ success: boolean; user?: ArtisanProfile; message?: string }> {
  ...
  async updateProfile(updates: Partial<ArtisanProfile>): Promise<ArtisanProfile> {
